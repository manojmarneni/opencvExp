...
class FrameProcessor {
  ...
  public:
	// processing method
	virtual void process(cv:: Mat &input, cv:: Mat &output, int *down, int *right,int *up, int *left)= 0;
};
...
class VideoProcessor {
  ...
  public:
	  ...
	  void run() {
		  ...
		  while (!isStopped()) {
			  ...
			  if (callIt) {
				...
				if (process)
					...
					frameProcessor->process(frame,output,&down,&right,&up,&left);
		  ...
		  std::cout << "down= "<< down <<" right = " << right <<" up = "<<up <<" left = "<< left;
