...
// processing method
	void process(cv:: Mat &frame, cv:: Mat &output,int *down,int *right,int *up, int *left) {
		...
		cv::calcOpticalFlowPyrLK(gray_prev, gray, // 2 consecutive images
            ...
            for( int i= 0; i < points[1].size(); i++ ) {
                ...
                if (acceptTrackedPoint(i, down, right, up, left)) {
	...
	void detectFeaturePoints() {
		...
		cv::vector<cv::KeyPoint> keypoints_object;
		detector.detect( gray, keypoints_object );

		cv::KeyPoint::convert(keypoints_object,features);
...
//			minDist);   // min distance between two features
	...
	bool acceptTrackedPoint(int i, int *down, int *right, int *up, int *left) {

		if(points[1][i].x - points[0][i].x > 2) (*right)++;
		if(points[0][i].x - points[1][i].x > 2) (*left)++;
		if(points[1][i].y-points[0][i].y > 2) (*down)++;
		if(points[0][i].y - points[0][1].x > 2) (*up)++;
